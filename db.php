...
class next_level_cache_wpdb extends wpdb
{
	...
	function prune_cache($persist = true)
	{
		...
		$limit = next_level_cache_wpdb::$MAX_CACHE_SIZE * 1024;
		
		if ($size > $limit) {
			
			// not using this currently due to problems with sorting fucking up the array keys
			// uksort($this->raw_cache['items'], array('next_level_cache_wpdb','cache_compare'));
	...
	/**
	 * This will schedule the cache to be persisted just before wordpress shuts down
	 */
...
/**
 * function declared globally so we can call it from a wordpress hook
 */
function next_level_cache_save_cache() {
	global $wpdb;
	$wpdb->save_cache_to_database();
}
 ...
 * @var wpdb
 */
if ((!$wpdb) || get_class($wpdb) != 'next_level_cache_wpdb') {
	$wpdb = new next_level_cache_wpdb( DB_USER, DB_PASSWORD, DB_NAME, DB_HOST );
}
