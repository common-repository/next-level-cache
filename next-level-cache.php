...
// display debug info in the footer only if specified in wp-config
if (defined('DEBUGQUERIES') && DEBUGQUERIES && defined('SAVEQUERIES') && SAVEQUERIES) {
	add_action('wp_footer', 'next_level_output_debug_sql', 1000);
}
if (defined('DEBUGOPTIONS') && DEBUGOPTIONS && defined('SAVEQUERIES') && SAVEQUERIES) {
	add_action('wp_footer', 'next_level_output_missing_options', 1001);
}
...
/**
 * output wordpress options that are missing from the wp_options table
 */
function next_level_output_missing_options()
{
	if (current_user_can( 'manage_options' ) && is_admin() == false) {
		...
		foreach ($wpdb->queries as $query) {
			...
			if (strpos($sql, 'SELECT option_value FROM wp_options WHERE option_name') !== false) {
				$count++;
				echo "-- " . $sql . "\n";
				echo str_replace(
						array('SELECT option_value FROM wp_options WHERE option_name = ',' LIMIT 1'),
						array('insert into wp_options (option_name,option_value,autoload) values(',",'','yes');"),
						$sql) . "\n";
			}
